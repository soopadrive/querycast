...
- **Stage 8 (✅ done — Polish + branding), sliced into 8a/8b/8c (+ optional 8d deferred):**
  ...
  - **8c (✅ done):** Accessibility pass: `:focus-visible` outlines (2px solid var(--blue), 2px offset) on every interactive surface so keyboard users see exactly where focus lives; `.section-toggle` headings get keyboard handlers (Enter/Space) to mirror click — without those, headings with `tabindex=0 role=button` were focusable but not activatable; settings drawer gains a Tab focus trap matching the modal player's; video cards become keyboard-actionable (`tabindex=0`, `role=button`, dynamic `aria-label="Play: <title>"`, Enter/Space mirrors click). Aria-label sweep — every button without visible text already had an explicit label; the dynamic ones (theme toggle, chip ×, pin star) update on state change. Copy review: no changes — existing wording matches the developer-user profile.
